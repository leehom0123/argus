...
class _Remote:
    ...
    def __init__(self, ssh_cfg: dict):
        ...
        self._proxy_cfg = ssh_cfg.get("proxy")
        self._client = None
        self._sftp = None
    ...
    def _make_sock(self):
        if not self._proxy_cfg:
            return None
        try:
            import socks  # noqa: WPS433 (PySocks)
        except ImportError:
            sys.exit(
                "[ERROR] PySocks is required for proxy support. "
                "Install with: pip install PySocks"
            )
        proxy_host = self._proxy_cfg["host"]
        proxy_port = int(self._proxy_cfg["port"])
